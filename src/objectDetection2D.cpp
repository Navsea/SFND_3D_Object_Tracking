...
void detectObjects(cv::Mat& img, std::vector<BoundingBox>& bBoxes, float confThreshold, float nmsThreshold, 
                   std::string basePath, std::string classesFile, std::string modelConfiguration, std::string modelWeights, bool bVis, cv::Mat& proc_img)
{
    ...
    if(bVis) {
        ...
        for(auto it=bBoxes.begin(); it!=bBoxes.end(); ++it) {
            ...
            cv::rectangle(proc_img, cv::Point(left, top), cv::Point(left+width, top+height),cv::Scalar(0, 255, 0), 2);
            
            string label = cv::format("%.2f", (*it).confidence);
            label = classes[((*it).classID)] + ":" + label;
            label = std::to_string(it->boxID) + label;
            label = "ID:" + label;
            ...
            rectangle(proc_img, cv::Point(left, top - round(1.5*labelSize.height)), cv::Point(left + round(1.5*labelSize.width), top + baseLine), cv::Scalar(255, 255, 255), cv::FILLED);
            cv::putText(proc_img, label, cv::Point(left, top), cv::FONT_ITALIC, 0.75, cv::Scalar(0,0,0),1);
            
        }
        
        //string windowName = "Object classification";
        //cv::namedWindow( windowName, 1 );
        //cv::imshow( windowName, proc_img );
